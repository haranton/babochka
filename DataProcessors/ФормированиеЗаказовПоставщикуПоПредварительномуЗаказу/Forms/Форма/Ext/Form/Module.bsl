...
Функция ПриИзменененииСоглашенияНаСервере(ЗаказОбъект, ЗаполнятьСоглашения = Ложь)
	...
	Если ЗаполнятьСоглашения Тогда
		...
		МодульЗакупки = ОбщегоНазначения.ОбщийМодуль("ЗакупкиСервер");
		Если МодульЗакупки <> Неопределено Тогда
			УсловияЗакупокПоУмолчанию = МодульЗакупки.ПолучитьУсловияЗакупокПоУмолчанию(ЗаказОбъект.Партнер, Новый Структура("УчитыватьГруппыСкладов, ВыбранноеСоглашение", Истина, ЗаказОбъект.Соглашение));
		КонецЕсли;
...
&НаКлиентеНаСервереБезКонтекста
Функция ПолучитьСтруктуруЗаполненияЦеныЗакупкиВСтрокеТЧ(Строка)
...
Функция ЗаполнитьЦеныВыделенныхСтрокПоВидуЦеныПоставщикаСервер(ЗаказОбъект, ВидЦеныПоставщика)
	...
	МодульЗакупки = ОбщегоНазначения.ОбщийМодуль("ЗакупкиСервер");
	Если МодульЗакупки <> Неопределено Тогда
		ЦеныРассчитаны = МодульЗакупки.ЗаполнитьЦены(
			Объект.ФормированиеЗаказов,
			МассивСтрок,
			Новый Структура( // Параметры заполнения
