...
Функция ТекстЗапросаТаблицаПреварительныеЗаказы(Запрос, ТекстыЗапроса, Регистры)
	...
	|	&Период КАК Период,
	|	ТаблицаТовары.Ссылка КАК ПредварительныйЗаказ,
	...
	|				И &Статус В (ЗНАЧЕНИЕ(Перечисление.СтатусыПредварительныхЗаказов.КПоступлению), ЗНАЧЕНИЕ(Перечисление.СтатусыПредварительныхЗаказов.Закрыт))
	...
	|		КОГДА НЕ ТаблицаТовары.Отменено
	|				И &Статус В (ЗНАЧЕНИЕ(Перечисление.СтатусыПредварительныхЗаказов.КПоступлению), ЗНАЧЕНИЕ(Перечисление.СтатусыПредварительныхЗаказов.Закрыт))
	|			ТОГДА ТаблицаТовары.СуммаИнвойс
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК СуммаКОформлениюИнвойс,
	|	ТаблицаТовары.СуммаИнвойс КАК СуммаЗаказаноИнвойс
	|ИЗ
	|	Документ.ПредварительныйЗаказ.Товары КАК ТаблицаТовары
	|ГДЕ
	...
	|	ТаблицаТовары.Номенклатура.ТоварнаяКатегория,
	|	ТаблицаТовары.Номенклатура.Пол,
	...
	|	0,
	|	-ТаблицаТовары.Количество,
	|	ТаблицаТовары.ПричинаОтмены,
	|	0,
	|	-ТаблицаТовары.СуммаИнвойс
	|ИЗ
	|	Документ.ПредварительныйЗаказ.Товары КАК ТаблицаТовары
	|ГДЕ
	|	ТаблицаТовары.Ссылка = &Ссылка
...
Функция ТекстЗапросаТаблицаПреварительныеЗаказыТоварныеКатегории(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ТаблицаТовары.СуммаИнвойс КАК СуммаЗаказаноИнвойс
	|ИЗ
	|	Документ.ПредварительныйЗаказ.ТоварныеКатегории КАК ТаблицаТовары
	|ГДЕ
	|	ТаблицаТовары.Ссылка = &Ссылка
	...
	|ИЗ
	|	Документ.ПредварительныйЗаказ.ТоварныеКатегории КАК ТаблицаТовары
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПредварительныйЗаказ.Товары КАК Товары
...
Функция ДанныеДокументаДляПроведения(Документ, Регистры, ДопПараметры = Неопределено) Экспорт
	...
	Если ТипЗнч(Документ) = Тип("ДокументОбъект.ПредварительныйЗаказ") Тогда
	...
	Иначе
		ДокументОбъект = ?(ТипЗнч(Документ) = Тип("ДокументСсылка.ПредварительныйЗаказ"), Документ.ПолучитьОбъект(), Неопределено);
	...
	ЗаполнитьПараметрыИнициализацииПредварительныхЗаказов(Запрос, ДокументСсылка);
...
Процедура ЗаполнитьПараметрыИнициализацииПредварительныхЗаказов(Запрос, ДокументСсылка)

	Запрос.УстановитьПараметр("Ссылка", ДокументСсылка);
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ПредварительныйЗаказ.Дата КАК Период,
	|	ПредварительныйЗаказ.Статус КАК Статус
	|ИЗ
	|	Документ.ПредварительныйЗаказ КАК ПредварительныйЗаказ
	|ГДЕ
	|	ПредварительныйЗаказ.Ссылка = &Ссылка";
	...
	Иначе
		...
		Запрос.УстановитьПараметр("Статус", ПредопределенноеЗначение("Перечисление.СтатусыПредварительныхЗаказов.ПустаяСсылка"));
	КонецЕсли;
...
Функция ДобавитьКомандуСоздатьНаОсновании(КомандыСозданияНаОсновании) Экспорт
	...
	Если ПравоДоступа("Добавление", Метаданные.Документы.ПредварительныйЗаказ) Тогда
		КомандаСоздатьНаОсновании = КомандыСозданияНаОсновании.Добавить();
		КомандаСоздатьНаОсновании.Идентификатор = Метаданные.Документы.ПредварительныйЗаказ.ПолноеИмя();
		КомандаСоздатьНаОсновании.Представление = ОбщегоНазначенияУТ.ПредставлениеОбъекта(Метаданные.Документы.ПредварительныйЗаказ);
...
Процедура РассчитатьСостояние(
	...
	|ИЗ
	|	Документ.ЗаказПоставщику КАК ВТ
	|ГДЕ
	|	ВТ.Ссылка = &ЗаказПоставщику
	|;
	|/////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВЫБОР
	...
	|		КОГДА (НЕ ДокументЗаказ.Проведен)
	|			ТОГДА ЛОЖЬ
	|		КОГДА (ДокументЗаказ.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовПоставщикам.Закрыт) И &ИспользоватьСтатусы)
	|			ТОГДА ЛОЖЬ
	...
	|ПО
	|	СостоянияЗаказовПоставщикам.Заказ = ДокументЗаказ.Ссылка
	|ГДЕ
	...
	Если ПроверитьЗаданияКОтражениюСостоянияЗаказов Тогда
		...
		ТекстЗапросаЕстьЗадания = СостоянияДокументов.ТекстЗапросаЕстьЗаданияКОтражениюСостоянияЗаказов("&ЗаказПоставщику");
	...
	Если ПравоДоступа("Чтение", Метаданные.РегистрыНакопления.РасчетыСКлиентами) Тогда
		...
		|			((ДокументЗаказ.Статус <> ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовПоставщикам.НеСогласован) И ДокументЗаказ.СуммаДокумента > 0)
		...
		|ПО
		|	ДокументЗаказ.ОбъектРасчетов = РасчетыСПоставщикамиОстатки.ОбъектРасчетов
		|";
	КонецЕсли;
...
// Заполняет адрес доставки по складу.
//
// Параметры:
// 	АдресДоставкиДляПоставщика - Строка - адрес доставки;
// 	ДоступныеСклады - массив из СправочникСсылка.Склады - Склады документа;
// 	ДатаПереодаС - Дата - дата начала действия адреса.
//
Процедура ЗаполнитьАдресДоставкиДляПоставщика(АдресДоставкиДляПоставщика, ДоступныеСклады, Знач ДатаПереодаС = Неопределено) Экспорт
	
	СписокВыбораАдреса = АдресаСкладов(ДоступныеСклады, ДатаПереодаС);
	
	Если СписокВыбораАдреса.Количество() = 1 Тогда
		АдресДоставкиДляПоставщика = СписокВыбораАдреса[0].Значение;
	КонецЕсли;
	
КонецПроцедуры

// Получает список адресов по складам для документа.
//
// Параметры:
// 	ДоступныеСклады - массив из СправочникСсылка.Склады - Склады документа;
// 	ДатаПереодаС - Дата - дата начала действия адреса.
//
// Возвращаемое значение:
//	СписокЗначений из Строка - данные адресов доставки.
//
Функция АдресаСкладов(ДоступныеСклады, Знач ДатаПереодаС = Неопределено) Экспорт
	
	СписокВыбораАдреса = Новый СписокЗначений;
	ДатаПереодаС = ?(ЗначениеЗаполнено(ДатаПереодаС), ДатаПереодаС, ТекущаяДатаСеанса());
...
Процедура ОбработкаПолученияФормы(ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация, СтандартнаяОбработка)
	...
	Если ВидФормы = "ФормаСписка" Тогда
		Если Не ПравоДоступа("Чтение", Метаданные.РегистрыСведений.СостоянияЗаказовПоставщикам)
			Или Не ПравоДоступа("Чтение", Метаданные.РегистрыСведений.СостоянияПоОбъектамУчетаЭДО) Тогда
...
Процедура ЗапланироватьПоступлениеТоваров(Запрос, ТекстыЗапроса, Регистры, ДокументОбъект)
	...
	Если ДокументОбъект <> Неопределено
		И НЕ (ДокументОбъект.Статус = Перечисления.СтатусыЗаказовПоставщикам.Подтвержден
	...
	Если ДокументОбъект = Неопределено Тогда
		ТекстЗапросаДокумента = СтрЗаменить(ТекстЗапросаДокумента, "ДанныеДокументаТаблицаТовары", "Документ.ЗаказПоставщику.Товары");
		ТекстЗапросаДокумента = СтрЗаменить(ТекстЗапросаДокумента, "ДанныеДокументаШапка", "Документ.ЗаказПоставщику");
		ТекстЗапросаДокумента = СтрЗаменить(ТекстЗапросаДокумента, "ШапкаДокумента.ДоговорВариантОформленияЗакупок", "ШапкаДокумента.Договор.ВариантОформленияЗакупок");
	КонецЕсли;
...
Процедура ДобавитьТекстыОтраженияВзаиморасчетов(Запрос, ТекстыЗапроса, Регистры, ДокументОбъект)
	...
	Если ДокументОбъект = Неопределено
		Или НЕ (ДокументОбъект.Статус <> Перечисления.СтатусыЗаказовПоставщикам.НеСогласован
	...
	Если ДокументОбъект = Неопределено
		Или Не (ДокументОбъект.ХозяйственнаяОперация <> Перечисления.ХозяйственныеОперации.ПриемНаХранениеСПравомПродажи
...
// 
// Параметры:
//  СсылкаОбъект - ДокументСсылка.ЗаказПоставщику - Документ, по которому нужно получить данные.
...
Функция СуммыПоЗаказам(СсылкаОбъект, ЗначенияРеквизитов = Неопределено) Экспорт
	...
	Если ЗначенияРеквизитов = Неопределено Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ТаблицаЗначений", "Документ.ЗаказПоставщику.Товары");
...
Процедура ПриПолученииПредопределенныхНаборовСвойств(Наборы) Экспорт
    ...
    Набор.Имя = "Документ_ЗаказПоставщикуКопия";
