#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
	...
	Процедура ЗаполнитьУсловияЗакупок(Знач УсловияЗакупок) Экспорт
		...
		Если Не ЗначениеЗаполнено(УсловияЗакупок.ИспользуютсяДоговорыКонтрагентов) 
			...
			КонецЕсли;
		Иначе
			РеквизытыДоговора = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Договор, "ОплатаВВалюте, ГруппаФинансовогоУчета");
			ОплатаВВалюте = РеквизытыДоговора.ОплатаВВалюте;
			ГруппаФинансовогоУчета = РеквизытыДоговора.ГруппаФинансовогоУчета;
		КонецЕсли;
	...
	Процедура ba_ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
		...
		Если Товары.Количество() > 0 Тогда
			...
			Если Статус = Перечисления.СтатусыПредварительныхЗаказов.Согласован
				ИЛИ Статус = Перечисления.СтатусыПредварительныхЗаказов.Подтвержден
				ИЛИ Статус = Перечисления.СтатусыПредварительныхЗаказов.КПоступлению
				ИЛИ Статус = Перечисления.СтатусыПредварительныхЗаказов.Закрыт Тогда
